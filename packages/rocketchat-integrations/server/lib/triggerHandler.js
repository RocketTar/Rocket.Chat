...
RocketChat.integrations.triggerHandler = new class RocketChatIntegrationHandler {
	...
	updateHistory({
		...
		if (historyId) {
			...
			return RocketChat.models.IntegrationHistory.insert(
				Object.assign({ _id: Random.id() }, history)
			);
		}
	}
	...
	sendMessage({ trigger, nameOrId = "", room, message, data }) {
		...
		if (!tmpRoom) {
			logger.outgoing.warn(
				`The Integration "${
					trigger.name
				}" doesn't have a room configured nor did it provide a room to send the message to.`
			);
			return;
		}

		logger.outgoing.debug(
			`Found a room for ${trigger.name} which is: ${
				tmpRoom.name
			} with a type of ${tmpRoom.t}`
		);
	...
	getIntegrationScript(integration) {
		...
		try {
			...
			logger.outgoing.error(
				`Error evaluating Script in Trigger ${integration.name}:`
			);
	...
	executeScript(integration, method, params, historyId) {
		...
		try {
			...
			logger.outgoing.debug(
				`Script method "${method}" result of the Integration "${
					integration.name
				}" is:`
			);
			...
			logger.outgoing.error(
				`Error running Script in the Integration ${integration.name}:`
			);
	...
	executeTriggers() {
		...
		logger.outgoing.debug(
			`Found ${
				triggersToExecute.length
			} to iterate over and see if the match the event.`
		);

		for (const triggerToExecute of triggersToExecute) {
			logger.outgoing.debug(
				`Is "${triggerToExecute.name}" enabled, ${
					triggerToExecute.enabled
				}, and what is the event? ${triggerToExecute.event}`
			);
			if (
	...
	executeTriggerUrl(
		...
		if (!this.isTriggerEnabled(trigger)) {
			logger.outgoing.warn(
				`The trigger "${
					trigger.name
				}" is no longer enabled, stopping execution of it at try: ${tries}`
			);
			return;
		}
		...
		if (RocketChat.integrations.outgoingEvents[event].use.triggerWords) {
			if (trigger.triggerWords && trigger.triggerWords.length > 0) {
				...
				if (!word) {
					logger.outgoing.debug(
						`The trigger word which "${
							trigger.name
						}" was expecting could not be found, not executing.`
					);
					return;
				}
			}
		...
		if (message && message.editedAt && !trigger.runOnEdits) {
			logger.outgoing.debug(
				`The trigger "${
					trigger.name
				}"'s run on edits is disabled and the message was edited.`
			);
			return;
		}
		...
		let opts = {
			...
			auth: undefined,
			npmRequestOptions: {
				rejectUnauthorized: !RocketChat.settings.get(
		...
		HTTP.call(opts.method, opts.url, opts, (error, result) => {
			if (!result) {
				...
				logger.outgoing.info(
					`Status code for the Integration ${trigger.name} to ${url} is ${
						result.statusCode
					}`
				);
			}
			...
			if (!result || !this.successResults.includes(result.statusCode)) {
				if (error) {
					logger.outgoing.error(
						`Error for the Integration "${trigger.name}" to ${url} is:`
					);
				...
				if (result) {
					...
					if (result.statusCode === 410) {
						...
						logger.outgoing.error(
							`Disabling the Integration "${
								trigger.name
							}" because the status code was 401 (Gone).`
						);
					...
					if (result.statusCode === 500) {
						...
						logger.outgoing.error(
							`Error "500" for the Integration "${trigger.name}" to ${url}.`
						);
					...
					if (tries < trigger.retryCount && trigger.retryDelay) {
						...
						logger.outgoing.info(
							`Trying the Integration ${
								trigger.name
							} to ${url} again in ${waitTime} milliseconds.`
						);
						Meteor.setTimeout(() => {
