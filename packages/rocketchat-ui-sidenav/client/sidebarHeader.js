...
const extendedViewOption = user => {
	if (RocketChat.settings.get("Store_Last_Message")) {
		return {
			...
			name: t("Extended"),
			modifier:
				RocketChat.getUserPreference(user, "sidebarViewMode") === "extended"
					? "bold"
					: null,
			action: () => {
				Meteor.call(
...
const toolbarButtons = user => [
	...
	},
	{
		...
		icon: () =>
			viewModeIcon[
				RocketChat.getUserPreference(user, "sidebarViewMode") || "condensed"
			],
		action: e => {
			const hideAvatarSetting = RocketChat.getUserPreference(
				user,
				"sidebarHideAvatar"
			);
			const config = {
	...
	},
	{
		...
		icon: "edit-rounded",
		condition: () =>
			RocketChat.authz.hasAtLeastOnePermission(["create-c", "create-p"]),
		action: () => {
			menu.close();
...
Template.sidebarHeader.events({
	...
	"click .sidebar__header .avatar"(e) {
		if (
			!(
				Meteor.userId() == null &&
				RocketChat.settings.get("Accounts_AllowAnonymousRead")
			)
		) {
