...
const getEmojis = function(collection, filter) {
	...
	if (
		!RocketChat.emoji.packages.emojione ||
		RocketChat.emoji.packages.emojione.asciiList[key]
	) {
		return [];
	}
		...
		const config = {
			...
			getFilter(collection, filterText = "", cb) {
				...
				if (items.length < 5 && filterText !== "") {
					...
					if (!RocketChat.authz.hasAllPermission("view-outside-room")) {
						const usernamesFromDMs = RocketChat.models.Subscriptions.find(
							{
								t: "d",
								$and: [
									{
						...
						const newItems = RocketChat.models.Users.find(
							{
								username: {
									$in: usernamesFromDMs
								}
