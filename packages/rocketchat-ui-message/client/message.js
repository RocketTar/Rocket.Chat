...
Template.message.helpers({
	...
	broadcast() {
		...
		return (
			!this.private &&
			!this.t &&
			this.u._id !== Meteor.userId() &&
			instance.room &&
			instance.room.broadcast
		);
	},
	isIgnored() {
		return this.ignored;
	...
	getName() {
		...
		const cleanName = this.u.name
			? this.u.name.split("/").pop()
			: this.u.username;

		return (
			(RocketChat.settings.get("UI_Use_Real_Name") && cleanName) ||
			this.u.username
		);
	},
	showUsername() {
		return (
			this.alias ||
			(RocketChat.settings.get("UI_Use_Real_Name") && this.u && this.u.name)
		);
	},
	own() {
	...
	chatops() {
		if (
			this.u &&
			this.u.username === RocketChat.settings.get("Chatops_Username")
		) {
	...
	showTranslated() {
		if (
			...
			this.u &&
			this.u._id !== Meteor.userId() &&
			!RocketChat.MessageTypes.isSystemMessage(this)
		) {
			...
			return (
				this.autoTranslateFetching ||
				(subscription &&
					subscription.autoTranslate !== this.autoTranslateShowInverse &&
					this.translations &&
	...
	canEdit() {
		const hasPermission = RocketChat.authz.hasAtLeastOnePermission(
			"edit-message",
			this.rid
		);
		...
		const blockEditInMinutes = RocketChat.settings.get(
	...
	canDelete() {
		...
		const blockDeleteInMinutes = RocketChat.settings.get(
	...
	hasOembed() {
		...
		if (
			...
			!RocketChat.settings.get("API_Embed")
		) {
			return false;
		}
		...
		if (
			(RocketChat.settings.get("API_EmbedDisabledFor") || "")
				.split(",")
				.map(username => username.trim())
				.includes(this.u && this.u.username)
		) {
			return false;
		}
		return true;
	...
	reactions() {
		...
		return Object.keys(this.reactions || {}).map(emoji => {
			...
			let usernames = reaction.usernames
				...
				.map(
					username =>
						username === userUsername ? t("You").toLowerCase() : `@${username}`
				)
				.join(", ");
			if (total > 15) {
	...
	messageActions(group) {
		...
		return RocketChat.MessageAction.getButtons(
...
Template.message.onCreated(function() {
	...
	this.wasEdited =
		msg.editedAt != null && !RocketChat.MessageTypes.isSystemMessage(msg);
	...
	return (this.body = (() => {
		...
		if (messageType.render) {
			...
			msg.u &&
			msg.u.username === RocketChat.settings.get("Chatops_Username")
		) {
			msg.html = msg.msg;
			console.log("MSG", msg);
...
Template.message.onViewRendered = function(context) {
	return this._domrange.onAttached(domRange => {
		...
		if (previousNode == null) {
			...
			if (
				previousDataset.groupable === "false" ||
				currentDataset.groupable === "false"
			) {
				$currentNode.removeClass("sequential");
			} else if (
				previousDataset.username !== currentDataset.username ||
				parseInt(currentDataset.timestamp) -
					parseInt(previousDataset.timestamp) >
					RocketChat.settings.get("Message_GroupingPeriod") * 1000
			) {
				$currentNode.removeClass("sequential");
		...
		if (nextNode && nextNode.dataset) {
			...
			if (nextDataset.groupable !== "false") {
				if (
					nextDataset.username !== currentDataset.username ||
					parseInt(nextDataset.timestamp) - parseInt(currentDataset.timestamp) >
						RocketChat.settings.get("Message_GroupingPeriod") * 1000
				) {
					$nextNode.removeClass("sequential");
				} else if (
					!$nextNode.hasClass("new-day") &&
					!$currentNode.hasClass("temp")
				) {
