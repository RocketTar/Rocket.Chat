...
export default class {
	...
	get pattern() {
		return typeof this._pattern === "function"
			? this._pattern()
			: this._pattern;
	}
	set useRealName(s) {
	...
	get useRealName() {
		return typeof this._useRealName === "function"
			? this._useRealName()
			: this._useRealName;
	}
	...
	replaceUsers(str, message, me) {
		return str.replace(this.userMentionRegex, (match, prefix, username) => {
			...
			const mentionObj =
				message.mentions &&
				message.mentions.find(m => m.username === username.trim());
			if (message.temp == null && mentionObj == null) {
				return match;
			}
			const name =
				this.useRealName && mentionObj && s.escapeHTML(mentionObj.name);

			return `${prefix}<a class="mention-link ${
				username === me ? "mention-link-me" : ""
			}" data-username="${username}" title="${name ? username : ""}">${name ||
				`@${username}`}</a>`;
		});
	}
	replaceChannels(str, message) {
		...
		return str
			.replace(/&#39;/g, "'")
			.replace(this.channelMentionRegex, (match, prefix, name) => {
				if (
					!message.temp &&
					!(message.channels && message.channels.find(c => c.name === name))
				) {
					return match;
				}
	...
	getChannelMentions(str) {
		return (str.match(this.channelMentionRegex) || []).map(match =>
			match.trim()
		);
	}
	parse(message) {
