...
export default class LDAP {
	constructor() {
		...
		this.options = {
			...
			reject_unauthorized:
				RocketChat.settings.get("LDAP_Reject_Unauthorized") || false,
			Authentication: RocketChat.settings.get("LDAP_Authentication"),
			Authentication_UserDN: RocketChat.settings.get(
				"LDAP_Authentication_UserDN"
			),
			Authentication_Password: RocketChat.settings.get(
			...
			group_filter_object_class: RocketChat.settings.get(
				"LDAP_Group_Filter_ObjectClass"
			),
			group_filter_group_id_attribute: RocketChat.settings.get(
				"LDAP_Group_Filter_Group_Id_Attribute"
			),
			group_filter_group_member_attribute: RocketChat.settings.get(
				"LDAP_Group_Filter_Group_Member_Attribute"
			),
			group_filter_group_member_format: RocketChat.settings.get(
				"LDAP_Group_Filter_Group_Member_Format"
			),
			group_filter_group_name: RocketChat.settings.get(
				"LDAP_Group_Filter_Group_Name"
			),
			find_user_after_login: RocketChat.settings.get(
	...
	getUserByIdSync(id, attribute) {
		...
		const Unique_Identifier_Field = RocketChat.settings
			.get("LDAP_Unique_Identifier_Field")
			.split(",");
	...
	authSync(dn, password) {
		...
		try {
			...
			if (this.options.find_user_after_login) {
				...
				if (result.length === 0) {
					logger.auth.info(
						"Bind successful but user was not found via search",
						dn,
						searchOptions
					);
					return false;
