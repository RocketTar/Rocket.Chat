...
class _Logger {
	constructor(name, config = {}) {
			...
			_.each(this.config.methods, (typeConfig, method) => {
				...
				this[method] = function(...args) {
					return self._log.call(self, {
						...
						level:
							typeConfig.level != null
								? typeConfig.level
								: defaultTypes[typeConfig.type] &&
								  defaultTypes[typeConfig.type].level,
						method,
						arguments: args
					});
				};
		...
		if (this.config.sections) {
			_.each(this.config.sections, (name, section) => {
				this[section] = {};
				_.each(defaultTypes, (typeConfig, type) => {
					self[section][type] = (...args) =>
						this[type].apply({ __section: name }, args);
					self[section][`${type}_box`] = (...args) =>
						this[`${type}_box`].apply({ __section: name }, args);
				});
				_.each(this.config.methods, (typeConfig, method) => {
					self[section][method] = (...args) =>
						self[method].apply({ __section: name }, args);
					self[section][`${method}_box`] = (...args) =>
						self[`${method}_box`].apply({ __section: name }, args);
				});
			});
		}
	...
	_log(options) {
		...
		if (options.box === true && _.isString(options.arguments[0])) {
			...
			const args = JSON.parse(JSON.stringify(options.arguments));
			if (
				options.arguments[1] === "sendMessage" &&
				typeof RocketChat !== "undefined" &&
				RocketChat.settings.get("Hide_Message_Field_In_Logs")
			) {
				args[options.arguments.length - 1]["0"].msg = "***secret***";
			}
...
Meteor.publish("stdout", function() {
	if (
		!this.userId ||
		RocketChat.authz.hasPermission(this.userId, "view-logs") !== true
	) {
		return this.ready();
	}
