...
function fetchRooms(userId, rooms) {
	if (
		!RocketChat.settings.get("Store_Last_Message") ||
		RocketChat.authz.hasPermission(userId, "preview-c-room")
	) {
		return rooms;
	}
...
Meteor.methods({
	spotlight(text, usernames, type = { users: true, rooms: true }, rid) {
		...
		const roomOptions = {
			limit: RocketChat.settings.get("Message_MaxRoomsInTagSearch"),
			fields: {
				t: 1,
				name: 1,
				lastMessage: 1
		...
		if (userId == null) {
			if (RocketChat.settings.get("Accounts_AllowAnonymousRead") === true) {
				result.rooms = fetchRooms(
					userId,
					RocketChat.models.Rooms.findByNameAndTypeNotDefault(
						regex,
						"c",
						roomOptions
					).fetch()
		...
		const userOptions = {
			limit: RocketChat.settings.get("Message_MaxUsersInTagSearch"),
			fields: {
				username: 1,
				name: 1,
				status: 1
		...
		if (RocketChat.authz.hasPermission(userId, "view-outside-room")) {
			if (
				type.users === true &&
				RocketChat.authz.hasPermission(userId, "view-d-room")
			) {
				result.users = RocketChat.models.Users.findByActiveUsersExcept(
					text,
					usernames,
					userOptions
				).fetch();
			...
			if (
				type.rooms === true &&
				RocketChat.authz.hasPermission(userId, "view-c-room")
			) {
				const searchableRoomTypes = Object.entries(
					RocketChat.roomTypes.roomTypes
				)
					.filter(roomType => roomType[1].includeInRoomSearch())
					.map(roomType => roomType[0]);

				const roomIds = RocketChat.models.Subscriptions.findByUserIdAndTypes(
					userId,
					searchableRoomTypes,
					{ fields: { rid: 1 } }
				)
				...
				result.rooms = fetchRooms(
					userId,
					RocketChat.models.Rooms.findByNameAndTypesNotInIds(
						regex,
						searchableRoomTypes,
						roomIds,
						roomOptions
